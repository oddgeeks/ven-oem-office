...
export class changeQuoteVendoStatusToSentExternally1667495261259
  implements MigrationInterface
{
  name = 'changeQuoteVendoStatusToSentExternally1667495261259';

  public async up(queryRunner: QueryRunner): Promise<void> {
    const pendingQuoteUsers = await queryRunner.query(
      `SELECT quote_id, user_id FROM oem.oem_quotes_users WHERE approval_status = 'Pending Customer Acceptance'`,
    );
    const pendingVendoUsers = await queryRunner.query(
      `SELECT vendo_id, user_id FROM oem.oem_vendos_users WHERE approval_status = 'Pending Customer Acceptance'`,
    );
    const pendingQuotes = await queryRunner.query(
      `SELECT quote_id FROM oem.oem_quotes WHERE quote_status = 'Pending Customer Acceptance'`,
    );
    const pendingVendos = await queryRunner.query(
      `SELECT vendo_id FROM oem.oem_vendos WHERE vendo_status = 'Pending Customer Acceptance'`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_vendos_users_approval_status_enum" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Sent Externally', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_vendos_vendo_status_enum" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Sent Externally', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_quotes_quote_status_enum" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Sent Externally', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_quotes_users_approval_status_enum" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Sent Externally', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
    ...
    if (pendingQuoteUsers.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_quotes_users SET approval_status = 'Sent Externally' WHERE quote_id IN (${pendingQuoteUsers
          .map((quoteUser: any) => quoteUser.quote_id)
          .join(',')}) AND user_id IN (${pendingQuoteUsers
          .map((quoteUser: any) => quoteUser.user_id)
          .join(',')})`,
    ...
    if (pendingVendoUsers.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_vendos_users SET approval_status = 'Sent Externally' WHERE vendo_id IN (${pendingVendoUsers
          .map((vendoUser: any) => vendoUser.vendo_id)
          .join(',')}) AND user_id IN (${pendingVendoUsers
          .map((vendoUser: any) => vendoUser.user_id)
          .join(',')})`,
    ...
    if (pendingQuotes.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_quotes SET quote_status = 'Sent Externally' WHERE quote_id IN (${pendingQuotes
          .map((quote: any) => quote.quote_id)
          .join(',')})`,
      );
    }
    if (pendingVendos.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_vendos SET vendo_status = 'Sent Externally' WHERE vendo_id IN (${pendingVendos
          .map((vendo: any) => vendo.vendo_id)
          .join(',')})`,
      );
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const pendingQuoteUsers = await queryRunner.query(
      `SELECT quote_id, user_id FROM oem.oem_quotes_users WHERE approval_status = 'Sent Externally'`,
    );
    const pendingVendoUsers = await queryRunner.query(
      `SELECT vendo_id, user_id FROM oem.oem_vendos_users WHERE approval_status = 'Sent Externally'`,
    );
    const pendingQuotes = await queryRunner.query(
      `SELECT quote_id FROM oem.oem_quotes WHERE quote_status = 'Sent Externally'`,
    );
    const pendingVendos = await queryRunner.query(
      `SELECT vendo_id FROM oem.oem_vendos WHERE vendo_status = 'Sent Externally'`,
    );
    if (pendingQuoteUsers.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_quotes_users SET approval_status = NULL WHERE quote_id IN (${pendingQuoteUsers
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_quotes_users_approval_status_enum_old" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Pending Customer Acceptance', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_quotes_quote_status_enum_old" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Pending Customer Acceptance', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
      `ALTER TABLE "oem"."oem_quotes" ALTER COLUMN "quote_status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_vendos_vendo_status_enum_old" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Pending Customer Acceptance', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
      `ALTER TABLE "oem"."oem_vendos" ALTER COLUMN "vendo_status" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "oem"."oem_vendos_users_approval_status_enum_old" AS ENUM('Draft', 'Pending Internal Approval', 'Auto-Approved', 'Approved', 'Pending Customer Acceptance', 'Rejected', 'Expired', 'Transacted')`,
    );
    await queryRunner.query(
    ...
    if (pendingQuoteUsers.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_quotes_users SET approval_status = 'Pending Customer Acceptance' WHERE quote_id IN (${pendingQuoteUsers
          .map((quoteUser: any) => quoteUser.quote_id)
          .join(',')}) AND user_id IN (${pendingQuoteUsers
          .map((quoteUser: any) => quoteUser.user_id)
          .join(',')})`,
    ...
    if (pendingVendoUsers.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_vendos_users SET approval_status = 'Pending Customer Acceptance' WHERE vendo_id IN (${pendingVendoUsers
          .map((vendoUser: any) => vendoUser.vendo_id)
          .join(',')}) AND user_id IN (${pendingVendoUsers
          .map((vendoUser: any) => vendoUser.user_id)
          .join(',')})`,
    ...
    if (pendingQuotes.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_quotes SET quote_status = 'Pending Customer Acceptance' WHERE quote_id IN (${pendingQuotes
          .map((quote: any) => quote.quote_id)
          .join(',')})`,
      );
    }
    if (pendingVendos.length > 0) {
      await queryRunner.query(
        `UPDATE oem.oem_vendos SET vendo_status = 'Pending Customer Acceptance' WHERE vendo_id IN (${pendingVendos
          .map((vendo: any) => vendo.vendo_id)
          .join(',')})`,
      );
    }
